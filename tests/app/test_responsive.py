...
def _open_category(page: Page) -> None:
    ...
    # Target the benchmark's own category by href: several categories are built
    # now, and the first link in the nav is not this one.
...
def test_multiple_categories_render(page: Page, app_url: str) -> None:
    """More than one category builds, so the summary aggregates across them.

    With a single category the overall summary table is a degenerate one-column
    weighted mean and the cross-category machinery (category weights, the
    benchmark-to-group aggregation) is never exercised. The fixture ships three
    categories precisely so this holds; assert it rather than trusting it.
    """
    _load(page, app_url, VIEWPORTS["desktop"])

    # The nav renders a beat after the start-up mask hides, so wait for the
    # first link rather than counting an empty locator.
    categories = page.locator('#sidebar-nav a[href^="/category/"]')
    expect(categories.first).to_be_visible(timeout=TIMEOUT)
    count = categories.count()
    assert count >= 2, f"expected >=2 categories in the nav, got {count}"

    # One "<name> Score" column per category, plus MLIP and the overall Score.
    headers = page.locator("#summary-table th.dash-header")
