...
def test_oc20neb(model_name: str) -> None:
    ...
    for reaction in reactions:
        ...
        neb = NEB(
            ...
            neb_class="DyNEB",
            n_images=8,
            neb_kwargs={"climb": False, "method": "improvedtangent", "scale_fmax": 1.0},
            interpolator="ase",
            minimize=True,
            minimize_kwargs={"fmax": 0.05, "steps": 300},
            optimizer=BFGS,
            plot_band=False,
            write_band=False,
        ...
        try:
            ...
            neb_result = neb.results_file

        except Exception as exc:
            warn(
                f"Error running NEB for {reaction} with {model_name} : {exc}",
                stacklevel=2,
            )

            out_dir = OUT_PATH / model_name
            out_dir.mkdir(exist_ok=True, parents=True)
            write(out_dir / f"{reaction}-neb-band.extxyz", [initial, final])
            neb.results = {"barrier": np.nan, "delta_E": np.nan, "max_force": np.nan}
            neb_result = out_dir / neb.results_file

        with open(neb_result, "w", encoding="utf8") as out:
