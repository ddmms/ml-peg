"""
Compute the NCIA D442x10 dataset for London dispersion.
...
class NCIAD442x10Benchmark(zntrack.Node):
    """Benchmark the NCIA_D442x10 dataset."""
    ...
    def run(self):
        ...
        data_path = (
            download_s3_data(
                filename="NCIA_D442x10.zip",
                key="inputs/non_covalent_interactions/NCIA_D442x10/NCIA_D442x10.zip",
            )
            / "NCIA_D442x10"
        )
...
def build_project(repro: bool = False) -> None:
    ...
    for model_name, model in MODELS.items():
        with project.group(model_name):
            benchmark = NCIAD442x10Benchmark(
                model=model,
