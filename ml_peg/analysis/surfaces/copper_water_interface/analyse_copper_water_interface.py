...
from ml_peg.analysis.surfaces.copper_water_interface import aml
from ml_peg.analysis.surfaces.copper_water_interface.decorators import (
    cell_to_bar,
    plot_hist,
)
...
def stdev_dipole_z_deviation() -> dict[str, dict]:
    ...
    -------
    dict[str, float]
        Dictionary of predicted dipole moment standard deviation differences
        for all models.
    """
    ...
    print(f"!!!Reference Dipole Stdev: {ref_dipole_stdev}")

    hist_results = {}

    def create_hist(data: np.ndarray) -> tuple[list, list]:
        """
        ...
        ----------
        data
            Data to create histogram from.

        Returns
        -------
        tuple[list, list]
            Tuple of bins and histogram counts.
        """
        bins = np.arange(-0.05, 0.05 + 1e-6, 0.0025)
        hist, _ = np.histogram(data, bins=bins, density=True)
        # Index each dipole into the bins
        indices = np.digitize(data, bins)
        # Select a dipole from each bin to represent the bin
        selected_bins = []
        for i in range(1, len(bins)):
            bin_indices = np.where(indices == i)[0]
            if len(bin_indices) > 0:
                selected_bins.append(data[bin_indices[0]])
            else:
                selected_bins.append(np.nan)

        # TODO use to sample structures to visualise later

        return {"bins": bins.tolist(), "hist": hist.tolist(), "indices": selected_bins}
    ...
    for model_name in MODELS:
        ...
        hist_results[model_name] = create_hist(dipoles_z)
        print(
            f"!!!Model: {model_name}, Dipole Stdev Deviation: {dipole_stdev},"
            f" number of samples: {len(dipoles_z)}"
        )

    # Add reference histogram
    hist_results["ref"] = create_hist(ref_dipoles)

    return {"stdev_dipole_z_deviation": dipole_results, "hist_data": hist_results}
...
def build_dipole_histogram(
    ...
    -------
    dict[str, dict]
        Dictionary of histogram data for all models.
    """
    return stdev_dipole_z_deviation["hist_data"]
...
def metrics(
    ...
    stdev_dipole_z_deviation: dict[str, dict],
    build_dipole_histogram: dict[str, dict],
) -> dict[str, dict]:
    """
    Get all metrics.
