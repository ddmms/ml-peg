...
class NCIANIHB100x10App(BaseApp):
    """NCIA IHB100x10 benchmark app layout and callbacks."""

    def register_callbacks(self) -> None:
        """Register callbacks to app."""
        scatter = read_plot(
            DATA_PATH / "figure_ncia_ihb100x10.json",
        ...
        if model_dir.exists():
            ...
            structs = [
                f"assets/non_covalent_interactions/NCIA_IHB100x10/{MODELS[0]}/{label}.xyz"
                for label in labels
            ]
        else:
            structs = []
...
def get_app() -> NCIANIHB100x10App:
    """
    Get NCIA_IHB100x10 benchmark app layout and callback registration.
    ...
    """
    return NCIANIHB100x10App(
        name=BENCHMARK_NAME,
        description=(
            "Performance in predicting intramolecular hydrogen bond energies "
            "for the NCIA IHB100x10 dataset (intramolecular conformers). "
