...
def build_nav(
    ...
    def toggle_benchmark(n_clicks: int, current: object) -> tuple[object, str]:
        """
        ...
        -------
        tuple[object, str]
            New body children and the header className reflecting the state.
        """
        key = ctx.triggered_id["index"]
        if current:
            return None, "mlpeg-bench-header"
        return (
    ...
    def set_all_benchmarks(
        ...
        -------
        tuple[list[object], list[str], str]
            New body children, header classNames, and the persisted preference.
        """
        ...
        if ctx.triggered_id == "expand-all-benchmarks" and expand_clicks:
            ...
            return children, [open_class] * len(body_ids), "expanded"
