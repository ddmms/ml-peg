...
def build_detail_loading_spinner() -> Div:
    """
    Build a spinner that reserves its own height.

    Unlike the table spinner (an absolute overlay), the click-to-view detail
    placeholders start empty, so an overlay would have no box to cover. This
    spinner sits in normal flow with a min-height, so it is visible while a slow
    detail render (e.g. a WEAS structure viewer) is in flight and collapses to
    nothing once the placeholder is empty again.

    Returns
    -------
    Div
        Centred, self-sized loading spinner.
    """
    return Div(
        _progress_ring("mlpeg-progress-ring--sm"),
        style={
...
def build_detail_overlay(placeholder_id: str, child: Component) -> Loading:
    """
    Wrap a click-to-view detail placeholder in a loading spinner.

    Shows the spinner while the cell/point-click callback renders the plot or
    structure viewer into ``placeholder_id``'s ``children``, so a slow render
    shows a buffer instead of an empty gap.

    Parameters
    ----------
    placeholder_id
        Id of the detail placeholder Div whose ``children`` the click callback
        populates.
    child
        The placeholder component to wrap.

    Returns
    -------
    Loading
        Loading wrapper scoped to the placeholder's children.
    """
    return Loading(
        child,
        fullscreen=False,
        custom_spinner=build_detail_loading_spinner(),
        target_components={placeholder_id: "children"},
        show_initially=False,
        # Skip the spinner for fast (pre-rendered plot) clicks; show it for the
        # slower structure-viewer renders.
        delay_show=150,
        delay_hide=100,
    )
...
def build_test_layout(
    ...
    # Collapsed by default: hides the (potentially ~90) weight/threshold inputs so
    # heavy pages render far faster; expand to adjust scoring. Collapsed <details>
    # content is display:none, so it isn't laid out/painted until opened.
    ...
    if extra_components:
        # Wrap click-to-view detail placeholders (plots / structure viewers) in a
        # loading spinner so a slow render shows a buffer instead of an empty gap.
        # Central choke point: every benchmark passes its placeholders here.
        # Preserve the upstream interactive-zone max-width (#720) around the set.
        wrapped = []
        for component in extra_components:
            component_id = getattr(component, "id", None)
            if isinstance(component_id, str) and component_id.endswith("-placeholder"):
                wrapped.append(build_detail_overlay(component_id, component))
            else:
                wrapped.append(component)
        layout_contents.append(
            Div(wrapped, style={"maxWidth": INTERACTIVE_ZONE_MAX_WIDTH})
        )
