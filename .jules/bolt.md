...
## 2026-07-27 - Iterating Pandas DataFrames
**Learning:** Using `iterrows()` to iterate over Pandas DataFrames is a performance bottleneck in the codebase.
**Action:** Replace `iterrows()` with `itertuples(index=False, name=None)` for significantly faster execution, or `to_dict('records')` when column names are dynamic or non-standard.
