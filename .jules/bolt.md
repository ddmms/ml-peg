...
## 2026-08-07 - Pandas Iteration Optimization
**Learning:** Iterating over Pandas DataFrames with `iterrows()` is a major performance bottleneck, especially for larger datasets, and generates FutureWarnings for position-based accesses on Series.
**Action:** Replace `iterrows()` with `itertuples(index=False, name=None)` for standard tuple-based iteration or `to_dict('records')` when dynamic dictionary-based access is required.
