...
## 2026-08-04 - Iterating DataFrames Performance
**Learning:** Iterating over Pandas DataFrames with `iterrows()` is a performance bottleneck.
**Action:** Replace with `itertuples(index=False, name=None)` for significantly faster execution if index-based lookups are needed. If dictionary access is required, especially with dynamic or non-standard column names, iterate over `df.to_dict('records')`.
