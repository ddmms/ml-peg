...
**Learning:** Iterating over Pandas DataFrames using `iterrows()` is a significant performance bottleneck due to type inference overhead and generating Series objects for each row.
**Action:** Replace `iterrows()` with `itertuples(index=False)` for standard namedtuple access (using dot notation), `itertuples(index=False, name=None)` for standard tuple access (integer-based indexing), or `to_dict('records')` when dynamic string-based column lookup is needed.
