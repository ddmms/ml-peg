...
## 2026-08-06 - Pandas iterrows() Performance Bottleneck
**Learning:** Iterating over Pandas DataFrames using `iterrows()` is a significant performance bottleneck due to the overhead of creating Pandas Series objects for every row.
**Action:** Replace `iterrows()` with `itertuples(index=False, name=None)` for standard tuple iteration, or `to_dict('records')` if dictionary key access (especially with dynamic variables) is required.
