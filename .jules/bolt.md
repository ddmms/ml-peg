...
## 2024-05-19 - [Pandas iterrows Performance]
**Learning:** Using `df.iterrows()` is extremely slow for iterating through Pandas DataFrames. Iterating via `df.itertuples(index=False, name=None)` or `df.to_dict('records')` (when string key access is necessary) is orders of magnitude faster.
**Action:** Always replace `iterrows()` with `itertuples()` or `to_dict('records')` across the codebase to avoid iteration bottlenecks.
