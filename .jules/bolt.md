...
## 2026-08-01 - Pandas Iteration Performance
**Learning:** `iterrows()` is a significant performance bottleneck for iterating over DataFrames due to yielding `(index, Series)`.
**Action:** Replace `iterrows()` with `itertuples(index=False, name=None)` for standard tuple lookup or `to_dict("records")` when dictionary access is required, as both are orders of magnitude faster.
