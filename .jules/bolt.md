...
**Action:** Replace `iterrows()` with `itertuples(index=False, name=None)` which yields standard tuples, or `to_dict('records')` if dictionary access is required (e.g. for dynamic columns), for significantly faster execution.
