...
## 2024-05-19 - DataFrame Iteration Performance
**Learning:** Iterating over Pandas DataFrames with `iterrows()` is a performance bottleneck.
**Action:** Replace `iterrows()` with `itertuples(index=False, name=None)` for standard tuples or `to_dict('records')` when dictionary access is required to significantly improve iteration speed.
