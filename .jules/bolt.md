...
## 2026-07-30 - Pandas DataFrame Iteration Optimization
**Learning:** Iterating over Pandas DataFrames with `iterrows()` is a major performance bottleneck due to type inference and Series creation per row.
**Action:** Always replace `iterrows()` with `itertuples(index=False, name=None)` for fast indexed access, or `to_dict("records")` when dynamic key lookups are required.
