...
## 2025-01-28 - Replace iterrows with itertuples and to_dict
**Learning:** Pandas `iterrows()` is a known performance bottleneck for iterating over DataFrames because it yields Series objects, incurring significant overhead per row.
**Action:** Use `itertuples(index=False, name=None)` for standard tuple iteration, or `to_dict('records')` when dictionary key access is needed, especially with dynamic column names.
