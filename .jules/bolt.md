...
## 2026-07-28 - Pandas iterrows iteration optimization
**Learning:** Iterating over Pandas DataFrames with `iterrows()` is a performance bottleneck. Replace with `itertuples(index=False, name=None)` for significantly faster execution returning standard tuples. If dictionary access is required, especially with dynamic or non-standard column names, iterate over `df.to_dict('records')`.
**Action:** Replaced `iterrows()` with `itertuples(index=False, name=None)` or `to_dict('records')`.
