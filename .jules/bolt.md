...
## 2026-05-06 - Caching YAML Load for FAQs registry
**Learning:** Just like `models.yml` and `frameworks.yml`, `faqs.yml` is repeatedly loaded via `yaml.safe_load` on app updates. This is a recurring micro-bottleneck in the app UI generation.
**Action:** Reused the `@functools.cache` and `deepcopy` pattern on `load_faqs_registry()` to avoid repetitive file I/O overhead.
