...
## 2026-08-02 - Pandas iteration performance bottleneck
**Learning:** Using `iterrows()` to iterate over Pandas DataFrames is a significant performance bottleneck due to series object creation overhead.
**Action:** Replace `iterrows()` with `itertuples(index=False, name=None)` for index-based access or `to_dict('records')` for dynamic dictionary access to dramatically improve iteration speed.
