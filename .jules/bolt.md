...
**Learning:** Iterating over Pandas DataFrames using `iterrows()` is a known performance bottleneck as it creates a Pandas Series for every row.
**Action:** Replace `iterrows()` with `itertuples(index=False, name=None)` when integer-based indexing is needed, or `to_dict('records')` when dictionary access with dynamic or potentially invalid identifiers is required.
