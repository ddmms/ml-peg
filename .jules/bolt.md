...
**Learning:** Iterating over Pandas DataFrames with `iterrows()` is a performance bottleneck because it yields slow-to-create `Series` objects.
**Action:** Replace with `itertuples(index=False, name=None)` for significantly faster execution if integer-based indexing is needed, or iterate over `df.to_dict('records')` if dictionary access is required (especially with dynamic/non-standard column names).
