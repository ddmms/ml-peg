...
**Learning:** Iterating over Pandas DataFrames with `iterrows()` creates slow pandas Series objects for every row, which becomes a performance bottleneck in benchmark parsing scripts (like conformers and elasticity benchmarks).
**Action:** Always use `itertuples(index=False)` for significantly faster execution. If integer-based column indexing is required (e.g. `row[0]`), use `itertuples(index=False, name=None)` which yields raw Python tuples. If dictionary access is required, access via `row._asdict()` or iterate via `df.to_dict('records')`.
