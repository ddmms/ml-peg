...
**Learning:** Iterating over pandas DataFrames with `.iterrows()` is a significant performance bottleneck due to series object creation overhead.
**Action:** Replace `.iterrows()` with `.itertuples(index=False, name=None)` for standard tuple returns or `.to_dict("records")` when dictionary/dynamic property access is needed.
