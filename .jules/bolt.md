...
## 2026-06-12 - Pandas DataFrame Iteration Bottleneck
**Learning:** Iterating over Pandas DataFrames with `iterrows()` is a significant performance bottleneck due to type inference and Pandas Series object creation overhead for every row.
**Action:** Replace `iterrows()` with `itertuples(index=False, name=None)` for standard tuple-based iteration (fastest) or `to_dict('records')` if dictionary-like access via dynamic keys is required.
