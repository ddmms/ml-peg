...
## 2024-07-10 - Refactoring Pandas DataFrame Iteration
**Learning:** Using `df.iterrows()` to iterate over Pandas dataframes is a performance bottleneck.
**Action:** Replace `iterrows()` with `df.itertuples(index=False, name=None)` for faster execution when integer-based indexing is needed, or iterate over `df.to_dict('records')` when dictionary access is required (especially with dynamic column names).
