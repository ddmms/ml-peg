...
## 2024-05-20 - Faster Pandas Iteration
**Learning:** Iterating over Pandas DataFrames with `iterrows()` is a performance bottleneck. It yields `(index, Series)` which has significant overhead.
**Action:** Replace `iterrows()` with `itertuples(index=False, name=None)` for significantly faster execution when integer-based indexing is acceptable. If dictionary access is required, especially with dynamic or non-standard column names, use `to_dict('records')`.
