...
**Learning:** `iterrows()` is a significant performance bottleneck in Pandas DataFrames, as it creates a new Series object for each row.
**Action:** Replace `iterrows()` with `itertuples(index=False, name=None)` for standard iteration or `to_dict('records')` when dictionary access with dynamic column names is needed.
