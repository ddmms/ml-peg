...
**Learning:** `df.iterrows()` is a known performance bottleneck in Pandas as it creates a new Series object for every single row, causing significant overhead in loops.
**Action:** Use `df.itertuples(index=False, name=None)` for fast tuple-based iteration, or `df.to_dict('records')` if dynamic or non-standard column key access is required.
