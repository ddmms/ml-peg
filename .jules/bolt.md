...
## 2024-05-20 - Replace iterrows with itertuples or to_dict
**Learning:** Iterating over Pandas DataFrames with `iterrows()` is a performance bottleneck. It yields a Series object, adding overhead.
**Action:** Replace `iterrows()` with `itertuples(index=False, name=None)` for integer-based indexing, or `df.to_dict('records')` if dictionary access is needed.
