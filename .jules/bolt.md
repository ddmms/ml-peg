...
**Learning:** Iterating over Pandas DataFrames with `iterrows()` is a performance bottleneck because it constructs a Series for each row.
**Action:** Replace `iterrows()` with `itertuples(index=False, name=None)` for position-based access or `to_dict('records')` for dictionary/string key access, especially when column names are dynamic.
