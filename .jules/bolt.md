...
## 2024-06-25 - Pandas DataFrame Iteration
**Learning:** Using `iterrows()` for iterating over pandas DataFrames is a known performance bottleneck due to creating a Series object per row. It can be significantly slower than other methods.
**Action:** Replace `iterrows()` with `itertuples(index=False, name=None)` for standard tuple iteration, or `to_dict("records")` when dynamic dictionary-like column access is required. Remember to adjust indexing logic appropriately.
