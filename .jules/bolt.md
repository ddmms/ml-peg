...
## 2026-08-09 - Faster Pandas Iteration
**Learning:** Iterating over Pandas DataFrames with `iterrows()` is a performance bottleneck and can raise `FutureWarning`s related to integer-based lookups on Pandas `Series` objects.
**Action:** Replace `iterrows()` with `itertuples(index=False, name=None)` for significantly faster execution. If dictionary access is required, especially with dynamic or non-standard column names, use `df.to_dict('records')`.
