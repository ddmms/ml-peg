...
## 2024-07-25 - Pandas DataFrame Iteration Performance
**Learning:** Iterating over Pandas DataFrames using `iterrows()` is a significant performance bottleneck because it yields a Series for each row, adding overhead.
**Action:** Replace `iterrows()` with `itertuples(index=False, name=None)` for standard tuple returns. If dictionary access is required, especially with dynamic column names, iterate over `df.to_dict('records')`.
